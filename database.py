        ...
        try:
            ...
            self.queries = {
                ...
                '48': {
                    ...
                    'desc': 'List all computer shortest paths to Enterprise Admins for Users, Domain Users, Authenticated Users, or Everyone groups',
                    'message_generator': None,
                    'handler': self.handle_path_query,
                    'type': 'general',
                },
                ...
                '105': {
                    'query': 'MATCH (n:User)-[r:DCSync]->(m:Domain) RETURN n.name ORDER BY n.name',
                    'desc': 'List all user DCSync privileges',
                    'message_generator': lambda r: (r.get('n.name') is not None) and f'{log.default}User {log.reset}{log.red}{r["n.name"]}{log.reset}{log.default} has DCSync privileges{log.reset}' or None,
                    'handler': self.handle_standard_query,
                    'type': 'general',
                },
                '106': {
                    'query': 'MATCH (n:Group)-[r:DCSync]->(m:Domain) RETURN n.name ORDER BY n.name',
                    'desc': 'List all group DCSync privileges',
                    'message_generator': lambda r: (r.get('n.name') is not None) and f'{log.default}Group {log.reset}{log.red}{r["n.name"]}{log.reset}{log.default} has DCSync privileges{log.reset}' or None,
                    'handler': self.handle_standard_query,
                    'type': 'general',
                },
                '107': {
                    'query': 'MATCH (n:Computer)-[r:DCSync]->(m:Domain) RETURN n.name ORDER BY n.name',
                    'desc': 'List all computer DCSync privileges',
                    'message_generator': lambda r: (r.get('n.name') is not None) and f'{log.default}Computer {log.reset}{log.red}{r["n.name"]}{log.reset}{log.default} has DCSync privileges{log.reset}' or None,
                    'handler': self.handle_standard_query,
                    'type': 'general',
                },
                '108': {
                    'query': 'MATCH (n)-[r:DCSync]->(m:Domain) RETURN n.name,labels(n) ORDER BY n.name',
                    'desc': 'List all DCSync privileges',
                    'message_generator': lambda r: (r.get('n.name') is not None) and f'{log.default}({r["labels(n)"][0]}/{r["labels(n)"][1]}) {log.red}{r["n.name"]}{log.reset}{log.default} has DCSync privileges{log.reset}' or None,
                    'handler': self.handle_standard_query,
                    'type': 'general',
                },
                '109': {
