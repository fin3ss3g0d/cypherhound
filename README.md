...
A `Python3` terminal application that contains 340+ `Neo4j` cyphers for BloodHound data sets and a `customqueries.json` file containing 324 cyphers for the GUI.
...
Almost all cyphers included in the terminal application (324 to be exact) have been ported over to `json` format for direct usage in the GUI. Follow the instructions below in order to begin using them!
